...
Bulk_Operators = [1.5113601, 3.78901, 6.99789631, 'Delta_e1', 'Delta_e2']
...
def evaluate(Matrix, delta_e1, delta_e2):
...
# def evaluateMat(Matrix, delta_e1):
# 	mat = np.zeros_like(Matrix, dtype=np.double)
# 	for index, expr in np.ndenumerate(Matrix):
...
delta_e1_range = np.linspace(12,14, delta_e1_num)
delta_e2_range = np.linspace(17,33, delta_e2_num)
...
for delta_e2_index, delta_e2 in np.ndenumerate(delta_e2_range):
	...
	for delta_e1_index, delta_e1 in np.ndenumerate(delta_e1_range):
		tempmat = evaluate(derMatrix_expr, delta_e1, delta_e2)
		log_zdata[delta_e2_index, delta_e1_index] = np.log(min(svdvals(tempmat)))

min_z = np.round(np.exp(np.amin(log_zdata)), 3)
ind = np.unravel_index(np.argmin(log_zdata, axis=None), log_zdata.shape)
[delta_e2_min, delta_e1_min] = [delta_e2_range[ind[0]], delta_e1_range[ind[1]]]
print('Minimum singular value of z = ', min_z, 'was found at delta_e1 = ', delta_e1_min, 'and delta_e2 = ', delta_e2_min)
...
# print("a_e*lambda_(sse) = {", params[:5]/N, "}")
# print("Boundary OPE coefficients:")
# print("mu_phi^2 = ", params[5]*(1/(N-1)))
# print("mu_(sD)^2 = ", params[6])
# print("a_s^2 = ", params[7])
...
plt.ylabel('log(z)')
ax = plt.gca()
plt.vlines(12.48153781, -9, -6, colors='black', linestyles='dashed')
