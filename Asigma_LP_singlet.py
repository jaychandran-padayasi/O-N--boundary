...
if load_EffDelta == 0:
	...
	Eff_Delta.append((N-1)*np.array(table1.bdy_block(dim-1)))
	...
	end_time = time.time() - start_time
	print('Done, took', np.round(end_time, 2), 'sec')
	np.savetxt('Eff_Delta_As.txt', Eff_Delta)
else:
	print('Loading Eff_Delta')
	Eff_Delta = np.loadtxt('Eff_Delta_As.txt')


c_T = np.array([N] + [0]*M_max)
b_ub = np.array([0]*(num_boundary+num_bulk+1))
b_eq = -1
A_eq = np.array([table1.identity_block()])
bound_array = [(1, 13)]
...
print(-1*np.dot(res.x, table1.identity_block()))